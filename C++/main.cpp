...
/*Struct for the time reversal portion*/
struct Domino{
...
std::array<float, 2>& reflect(std::array<float,2> &vec, Wall &wall){
    /* Calculate reflection of vector with a wall*/
...
Hit_data& collision(std::array<float,2> &pos, std::array<float,2> &vec, Wall &wall, float det){
    /* Function that calculates collision point*/
...
void simulate(std::vector<std::array<float, 2>> &points, 
    ...
    static const float NORM_TOL2 = pow(10,-5);
    ...
    int hit_ind; float wall_scalar; int hit_part_id;
    ...
    while (COUNTER < MAX_ITERS){
        ...
        hit_norm = BASE_NORM;
        int prev_part_wall = unconf_walls[ap];
        unconf_walls[ap] = -1;
        ...
        for (int i = 0; i < COUNTER; i++){
            ...
            /* Ignore previous wall of particle and last hit wall -> these are impossible to hit*/
            if (i == prev_part_wall || i == reflect_indices[ap]){
                continue;
            }

            /* Ignore wall if particle should ignore it except when it is unconfirmed -> could still reverse it*/
            if (interactions[ap][wall.id] == 1 && unconf_walls[wall.id] != i) {
                continue;
            }

            /* Calculate determinant for collision -> if 0 collision not possible*/
            ...
            if (cnorm < hit_norm){
                hit_norm = cnorm;
                hit_position = hit_data.vect; hit_ind = i; wall_scalar = hit_data.scalar; hit_part_id = wall.id;
            }
        }
        /* NO collisions found*/
        ...
        if (hit_ind == unconf_walls[hit_part_id]){
            int j = hit_part_id;
            ...
            if (collision_time > particle_times[ap] + hit_norm && interactions[j][ap] == 0){
                /*update wall*/
        ...
        unconf_walls_times[ap] = {particle_times[ap], hit_norm};
        ...
        if (hit_norm == 0 || (hit_norm < NORM_TOL2 && reflect_indices[ap] != -1)){
            particle_times[ap] += TIME_LIMIT;
        }
        /* increment counter */
        COUNTER ++;
    ...
    /*Calculate MSD*/
    std::vector<float> MSD(N_particles, 0);

    for (int i = 0; i < N_particles; i++){
        MSD[i] = sqrt(pow((start_positions[i][0] - positions[i][0]), 2) + pow((start_positions[i][1] - positions[i][1]), 2));     
    }
    ...
    for (int i = 0; i < N_particles; i++){
        ...
        // Distance data
        DistData << Particle_Dist[i] << "," << MSD[i] << ";";
        //for (int j = 0; j<data_size; j++){
        //        PathData << POS_HIST[i][j][0] << "," << POS_HIST[i][j][1]<< "; ";
        //    }
        //    PathData << "\n";
        }
    ...
    //PathData << "\n";
    /*
    //DataFile.open("data.txt", std::ios::out | std::ios::app);
    DataFile.open("data.txt", std::ios::out);
    EndPoints.open("endpoints.txt", std::ios::out | std::ios::app);
    for (int i = 0; i < N_particles; i++){
        int data_size = POS_HIST[i].size();
        for (int j = 0; j<data_size; j++){
            DataFile << POS_HIST[i][j][0] << "," << POS_HIST[i][j][1]<< "; ";
            if (j == data_size-1){
                EndPoints << POS_HIST[i][j][0] << "," << POS_HIST[i][j][1]<< "; ";
            }
        }
        DataFile << "\n";
        }
...
{    
    ...
    /* Base file names*/
    std::string PathBase = "pathdata.txt";
    std::string PosBase = "posdata.txt";
    std::string DistBase = "distdata.txt";

    std::array<float, 5> SpeedValues = {0.5, 1, 2, 4, 16};

    /* Data files */
    static std::ofstream DataFile;
    static std::ofstream DistData;
    static std::ofstream PathData;

    // Open data files
    DataFile.open(PosBase, std::ios::out);
    DistData.open(DistBase, std::ios::out);
    PathData.open(PathBase, std::ios::out);
    ...
    // Stuff for  printing status of simulations
    int N = points.size();
    ...
    for (int sim = 0; sim < N_SIMULATIONS; sim++){
        ...
        // print status of simulations
        if (sim % sim_status_data == 0){
            std::cout << 10*(sim/sim_status_data) << "%, ";
        }
        //PathData << "// sim: "  << sim+1 << "\n";
        simulate(points, N, N_particles, BOUNDARY_LINES, start_positions, start_vectors, interactions, DataFile, DistData, PathData, 15);
    }
